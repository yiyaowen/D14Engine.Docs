...
1. 引入的 UI 库过于臃肿，特别是开发环境，甚至达到了数 GBs 的规模，而命令行程序本身却并不庞大。
2. 即使该 UI 库足够轻量，却避免不了繁琐的环境搭建，添加 GUI 比实现程序原本的功能更加让人烧脑。
3. 许多历史悠久的 UI 库虽然功能丰富，但控件的外观还停留在远古时代，想要从零开始设计并不简单。

相比之下，D14UIKit 内部的框架基于 Win32 原生窗口，所有的图形绘制均通过 DirectX 实现，没有复杂的外部依赖，导出的库文件很小。如果你想为某个运行在 Windows 平台上的小型项目提供可视化界面，同时又不希望引入的 UI 库喧宾夺主时，D14UIKit 是一个值得考虑的选择。
